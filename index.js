...
function addMetadata(packname, author) {	
	if (!packname) packname = '@VIBUOFC'; if (!author) author = 'N1V0';	
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Calmao causa estoy procesando😎\n\n❗Por favor no hacer spam👏❗\n\nVIBU OFC',
				success: '✔️ Listo ✔️',
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5112773927@s.whatsapp.net'
...
//FUNCION ANTILINK
	...
	if (budy.includes("https://www.facebook.com/")){
		...
		if (!isAntiFace) return
                if (isGroupAdmins) return reply('Eres un administrador del grupo, así que no te prohibiré el uso de enlaces :d')
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
/******JUEGOS SHANDUY LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
	       client.sendMessage(from, {displayname: "Vibu", vcard: vcard}, MessageType.contact, { quoted: mek})
		client.sendMessage(from, 'Arriba está el número del creador del bot <V I B U  O F C>\n\nNO SOY UN BOT LPM 🥸\n\nAhi puedes resolver tus preguntas y errores :)\n\nEste no es el numero del propietario del bot que estas usando ahora mismo. Si no, del creador de la base de datos del bot o sea Vibu\n\nV*10',MessageType.text, { quoted: mek} )
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 VIBU LEGEND 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
